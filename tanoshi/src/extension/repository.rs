...
pub async fn get_sources() -> Result<Vec<Source>, rusqlite::Error> {
    let db = connect_db();
    let mut stmt = db.prepare("SELECT id, name, url, version FROM source")?;
    let sources = stmt
        .query_map(params![], source_from_row)?
        .filter_map(|s| s.ok())
        .collect();
    Ok(sources)
}

pub async fn get_source(source_id: i32) -> Result<Source, rusqlite::Error> {
    let db = connect_db();
...
pub async fn get_source_from_manga_id(manga_id: i32) -> Result<Source, rusqlite::Error> {
    let db = connect_db();
    let ret = db.query_row(
...
pub async fn get_image_from_page_id(page_id: i32) -> Result<Image, rusqlite::Error> {
    let db = connect_db();
    let ret = db.query_row(
        r#"SELECT
        s.id AS source_id,
...
pub async fn get_source_from_chapter_id(chapter_id: i32) -> Result<Source, rusqlite::Error> {
    let db = connect_db();
    let ret = db.query_row(
        r#"SELECT source.id, source.name, source.url, source.version
        FROM chapter
...
pub async fn get_manga_url(manga_id: i32) -> Result<String, rusqlite::Error> {
    let db = connect_db();
    let ret = db.query_row(
...
pub async fn get_chapter_url(chapter_id: i32) -> Result<String, rusqlite::Error> {
    let db = connect_db();
    let ret = db.query_row(
...
pub async fn get_pages(chapter_id: i32) -> Result<GetPagesResponse, rusqlite::Error> {
    let db = connect_db();
...
pub async fn insert_sources(sources: Vec<Source>) -> Result<(), rusqlite::Error> {
...
pub async fn insert_mangas(source_id: i32, mangas: Vec<Manga>) -> Result<Vec<i32>> {
    let mut ids = vec![];
    let mut db = connect_db();
...
pub async fn insert_chapters(manga_id: i32, chapters: Vec<Chapter>) -> Result<(), rusqlite::Error> {
    let mut db = connect_db();
    let mut tx = db.transaction()?;
    for c in chapters {
        tx.execute(
...
pub async fn insert_pages(chapter_id: i32, pages: Vec<String>) -> Result<(), rusqlite::Error> {
    let mut db = connect_db();
    let tx = db.transaction()?;
...
pub async fn update_manga_info(manga_id: i32, manga: Manga) -> Result<(), rusqlite::Error> {
    let db = connect_db();
    db.execute(

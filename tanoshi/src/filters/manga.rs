...
pub fn manga(secret: String, exts: Arc<RwLock<Extensions>>) -> BoxedFilter<(impl Reply,)> {
    list_sources(exts.clone())
        .or(list_mangas(secret.clone(), exts.clone()))
        .or(get_manga_info(secret.clone(), exts.clone()))
        .or(get_chapters(secret.clone(), exts.clone()))
        .or(get_pages(exts.clone()))
        .or(proxy_image(exts.clone()))
        .or(source_login(exts.clone()))
        .boxed()
}

pub fn list_sources(exts: Arc<RwLock<Extensions>>) -> BoxedFilter<(impl Reply,)> {
...
pub fn list_mangas(secret: String, exts: Arc<RwLock<Extensions>>) -> BoxedFilter<(impl Reply,)> {
...
pub fn get_manga_info(secret: String, exts: Arc<RwLock<Extensions>>) -> BoxedFilter<(impl Reply,)> {
...
pub fn get_chapters(secret: String, exts: Arc<RwLock<Extensions>>) -> BoxedFilter<(impl Reply,)> {
...
pub fn get_pages(exts: Arc<RwLock<Extensions>>) -> BoxedFilter<(impl Reply,)> {
...
pub fn proxy_image(exts: Arc<RwLock<Extensions>>) -> BoxedFilter<(impl Reply,)> {
